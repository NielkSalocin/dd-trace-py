...
from ddtrace.llmobs._constants import INTENT
...
class MCPIntegration(BaseLLMIntegration):
    ...
    # Call before the tool is called or the input is recorded
    def process_ddtrace_argument(self, span: Span, request: "CallToolRequestType") -> None:
        ...
        if isinstance(arguments, dict) and ddtrace:
            ...
            if intent:
                span._set_ctx_item(INTENT, intent)
    ...
    def _llmobs_set_tags_request_responder_respond(
        ...
        span._set_ctx_items(
            {NAME: span_name, SPAN_KIND: span_kind, INPUT_VALUE: safe_json(input_obj), OUTPUT_VALUE: output_value}
        )
        if custom_tags:
